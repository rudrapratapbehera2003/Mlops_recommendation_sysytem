...
def load_all_courses(data_path: str = "data/processed/engineered_recommend_data.csv") -> pd.DataFrame:
...
def _recommend_for_cbf(
    ...
    selected_model = bundle.get("models", {}).get("cbf") if isinstance(bundle.get("models"), dict) else bundle
    model = selected_model.get("model") if isinstance(selected_model, dict) else None
    preprocessor = selected_model.get("preprocessor") if isinstance(selected_model, dict) else None
    feature_cols = selected_model.get("feature_cols", []) if isinstance(selected_model, dict) else []
    recommendations: List[Dict[str, Any]] = []

    if not hasattr(model, "predict"):
        return recommendations
    if preprocessor is None or not feature_cols:
        raise ValueError("Content-based model bundle is missing the trained preprocessor and feature columns.")
    ...
    for _, course in available_courses.iterrows():
        feature_dict = {
            ...
            "previous_courses_taken": user_preferences.get('previous_courses_taken', int(course.get('previous_courses_taken', 0))),
...
def _recommend_for_cf(
    ...
    selected_model = bundle.get("models", {}).get("cf") if isinstance(bundle.get("models"), dict) else bundle
    model_meta = selected_model.get("model", {}) if isinstance(selected_model, dict) else {}
    pred_matrix = model_meta.get("pred_matrix") if isinstance(model_meta, dict) else None
...
def predict_recommendations(
    ...
    selected_model_type = model_type or bundle.get("selected_model") or "content_based_random_forest"

    if user_preferences is not None and user_courses_taken is not None:
        if selected_model_type == "content_based_random_forest":
            recommendations = _recommend_for_cbf(bundle, user_id, top_n, user_preferences, user_courses_taken)
        elif selected_model_type == "collaborative_filtering":
            recommendations = _recommend_for_cf(bundle, user_id, top_n, user_courses_taken)
        else:
        ...
        if selected_model_type == "content_based_random_forest":
            recommendations = _recommend_for_cbf_from_candidates(bundle, user_id, top_n, candidate_features or [])
        elif selected_model_type == "collaborative_filtering":
            recommendations = _recommend_for_cf_from_candidates(bundle, user_id, top_n, candidate_course_ids or [])
        else:
            raise ValueError(f"Unsupported model type: {selected_model_type}")
    else:
        raise ValueError("Must provide either user_preferences+user_courses_taken OR candidate_features/candidate_course_ids")
