...
class FeatureEnginner:
    ...
    def cliping_outliers(self, df: pd.DataFrame) -> pd.DataFrame:
        logger.info(
            "Starting the clipping of outliers in the numerical variables...... "
        )
        ...
        logger.info(
            f"Clipping of the outliers in numerical column is completed successfully with summary: {outlier_summary}"
        )

        logger.info(
            "Further checking whether the outliers have been clipped or not....."
        )
    ...
    def run_feature_engineering(self):
        ...
        logger.info(
            f"Feature enginnering successfull. Matrix saved to: {self.feature_output_path}"
        )
        return clean_df
