...
def validate_data_task(**context):
    """Validate the processed data generated by the ingestion task."""
    logger.info("Starting data validation...")
    try:
        config = ConfigLoader()
        validator = DataValidator(config)
        validation_passed = validator.run()
        logger.info("Validation result: %s", validation_passed)
...
def ingest_data_task(**context):
    ...
    try:
        ...
        processed_df = ingestion.run_ingestion_pipeline()
        processed_data_path = config.get("data.processed_data_path")

        context['ti'].xcom_push(key='ingestion_status', value='completed')
        logger.info("Data ingestion completed successfully: %s rows", len(processed_df))
...
def model_registry_task(**context):
    ...
    try:
        ...
        if not best_model or not mlflow_run_id or not model_path:
            raise ValueError("Training task did not return complete model metadata.")

        logger.info("Model %s registered successfully in MLflow", best_model)
